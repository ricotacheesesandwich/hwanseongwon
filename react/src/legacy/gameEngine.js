...
(() => {
  ...
  const INFECTION_STAGE_EFFECTS = [
    {
      name: "0단계 · 초기 증상",
      icon: "◇",
      description: "미세한 한기와 갈증이 느껴집니다.",
    },
    {
      name: "1단계 · 경도 감염",
      icon: "◇",
      description: "복통, 구토, 손발 저림이 나타납니다.",
    },
    {
      name: "2단계 · 진행 감염",
      icon: "◇",
      description: "체온 저하, 관절 경직, 결정화가 진행됩니다.",
    },
    {
      name: "3단계 · 중증 감염",
      icon: "◇",
      description: "감각 둔화, 운동장애, 환청, 기억 혼선이 나타납니다.",
    },
    {
      name: "4단계 · 위중 감염",
      icon: "◇",
      description:
        "장기 기능 저하, 의식 단절, 가사동결이 진행되며 사망이 임박합니다.",
    },
    {
      name: "5단계 · 빙혼 완료",
      icon: "◇",
      description: "생명활동이 정지하고 동결체화가 완료됩니다.",
    },
  ];
  ...
  function applySessionTheme() {
    ...
    if (themeMeta)
      themeMeta.setAttribute(
        "content",
        spiritThemeActive ? "#16090c" : "#0c2744",
      );
  }
  ...
  function handleModalClick(event) {
    ...
    const removeCustomStatusButton = event.target.closest(
      "[data-remove-custom-status]",
    );
    if (removeCustomStatusButton) {
      const character = getCharacter(
        Number(removeCustomStatusButton.dataset.characterId),
      );
      if (character) {
        character.customStatuses = (character.customStatuses || []).filter(
          (status) =>
            status.id !== removeCustomStatusButton.dataset.removeCustomStatus,
        );
        addLog(
          `관리자가 ${character.name}의 직접 입력 상태이상을 삭제했습니다.`,
        );
        persistState();
        renderAll();
        if (ui.operationsOpen) renderAdminOperationsPage();
        showCharacterStatusEditor(character.id);
      }
      return;
    }
  ...
  function handleRightSidebarSubmit(event) {
    const customStatusForm = event.target.closest("[data-custom-status-form]");
    if (customStatusForm && session?.type === "admin") {
      event.preventDefault();
      const formData = new FormData(customStatusForm);
      ...
      if (!bodyPart && !severity && !detail)
        return showToast("다친 부위, 정도 또는 상세 내용을 입력해 주세요.");
      if (!Array.isArray(character.customStatuses))
        character.customStatuses = [];
      character.customStatuses.push({
        id: `custom-status-${Date.now()}-${Math.random().toString(36).slice(2, 7)}`,
        bodyPart,
        severity,
        detail,
      ...
      addLog(
        `관리자가 ${character.name}에게 직접 입력 상태이상을 추가했습니다.`,
      );
      persistState();
      renderAll();
      if (ui.operationsOpen) renderAdminOperationsPage();
  ...
  function characterStatusSettingsMarkup() {
    const rows = state.characters
      .map((character) => {
        const customStatuses = character.customStatuses || [];
        const conditionText = customStatuses.length
          ? customStatuses
              .map(
                (status) =>
                  [status.bodyPart, status.severity]
                    .filter(Boolean)
                    .join(" · ") || status.detail,
              )
              .filter(Boolean)
              .join(", ")
          : "추가 상태 없음";
        return `<tr><td><strong>${character.id} · ${escapeHtml(character.name)}</strong></td><td>${roleChipMarkup(character.role)}</td><td>${escapeHtml(conditionText)}</td><td><button type="button" class="button button--small" data-edit-character-status="${character.id}">수정</button></td></tr>`;
      })
      .join("");
    return `<section class="operations-card"><header><div><p class="eyebrow">CHARACTER CONDITION</p><h2>캐릭터 상태이상 관리</h2></div><span>${state.characters.length}명</span></header><p class="form-help">감염 단계에 따른 증상은 자동으로 표시됩니다. 여기서는 다친 부위와 정도 등 운영진이 직접 기록할 상태이상을 관리합니다.</p><div class="operations-table-wrap"><table class="operations-table"><thead><tr><th>ID · 이름</th><th>분류</th><th>관리자 추가 상태</th><th>수정</th></tr></thead><tbody>${rows}</tbody></table></div></section>`;
  }

  function showCharacterStatusEditor(characterId) {
    const character = getCharacter(characterId);
    if (!character) return;
    ui.selectedCharacterId = character.id;
    const customStatuses = character.customStatuses || [];
    const statusList = customStatuses.length
      ? customStatuses
          .map((status) => {
            const title =
              [status.bodyPart, status.severity].filter(Boolean).join(" · ") ||
              "관리자 추가 상태";
            return `<div class="status-list__item"><div><strong>＋ ${escapeHtml(title)}</strong>${status.detail ? `<p>${escapeHtml(status.detail)}</p>` : ""}</div><button type="button" class="button button--small button--danger" data-remove-custom-status="${escapeHtml(status.id)}" data-character-id="${character.id}">삭제</button></div>`;
          })
          .join("")
      : emptyStateMarkup("관리자가 추가한 상태이상이 없습니다.");

    openModal({
      eyebrow: "CHARACTER CONDITION",
      title: `${character.name} · 상태이상 수정`,
      body: `<div class="status-list">${statusList}</div><form class="operations-form" data-custom-status-form><input type="hidden" name="characterId" value="${character.id}"><div class="operations-form-grid"><label>다친 부위<input class="form-control" name="bodyPart" maxlength="50" placeholder="예: 왼쪽 팔, 갈비뼈"></label><label>정도<input class="form-control" name="severity" maxlength="40" placeholder="예: 찰과상, 중상, 골절 의심"></label></div><label>상세 내용<textarea class="form-control" name="detail" rows="4" maxlength="240" placeholder="관찰된 증상이나 처치 내용을 입력하세요."></textarea></label><button type="submit" class="button button--primary">상태이상 적용</button></form>`,
      footer: `<button type="button" class="button" data-modal-close>닫기</button>`,
    });
  }

  function settingsOperationsMarkup() {
    return `<form data-exposure-settings-form><div class="settings-role-grid">${roleSettingsMarkup("survivor")}${roleSettingsMarkup("spirit")}</div><section class="operations-card settings-guide"><h3>노출 설정 원칙</h3><p>체크박스를 조정한 뒤 저장 버튼을 눌러야 적용됩니다. 공간 진행도와 빙혼 시간 배율은 운영진에게만 공개됩니다.</p></section><div class="settings-save-bar"><span class="muted-text">변경사항은 저장 전까지 적용되지 않습니다.</span><button type="submit" class="button button--primary">환경설정 저장 및 적용</button></div></form>${characterStatusSettingsMarkup()}`;
  }
  function roleSettingsMarkup(role) {
    const exposure = getRoleExposure(role);
  ...
  function handleOperationsClick(event) {
    ...
    const statusEditor = event.target.closest("[data-edit-character-status]");
    if (statusEditor) {
      showCharacterStatusEditor(
        Number(statusEditor.dataset.editCharacterStatus),
      );
      return;
    }
  ...
  function ensureFeatureState(candidate) {
    ...
    next.characters.forEach((character) => {
      ...
      if (!Array.isArray(character.customStatuses))
        character.customStatuses = [];
      character.customStatuses = character.customStatuses
        .filter((status) => status && typeof status === "object")
        .map((status) => ({
          id: String(
            status.id ||
              `custom-status-${Date.now()}-${Math.random().toString(36).slice(2, 7)}`,
          ),
          bodyPart: String(status.bodyPart || "").trim(),
          severity: String(status.severity || "").trim(),
          detail: String(status.detail || "").trim(),
          createdAt: status.createdAt || new Date().toISOString(),
        }))
        .filter(
          (status) => status.bodyPart || status.severity || status.detail,
        );
      if (!Array.isArray(character.records)) character.records = [];
      if (!Array.isArray(character.investigations))
        character.investigations = [];
  ...
  function refreshLiveInfectionClocks() {
    ...
    document
      .querySelectorAll("[data-infection-status-effect]")
      .forEach((node) => {
        const character = getCharacter(
          Number(node.dataset.infectionStatusEffect),
        );
        if (!character || character.role !== "survivor") return;
        const effect =
          INFECTION_STAGE_EFFECTS[
            freezeStage(effectiveFreezeHours(character))
          ] || INFECTION_STAGE_EFFECTS[0];
        node.innerHTML = `<strong>${effect.icon} ${escapeHtml(effect.name)}</strong><p>${escapeHtml(effect.description)}</p>`;
      });
  }
  ...
  function infectionStageStatusMarkup(character) {
    if (!character || character.role !== "survivor") return "";
    const stage = freezeStage(effectiveFreezeHours(character));
    const effect = INFECTION_STAGE_EFFECTS[stage] || INFECTION_STAGE_EFFECTS[0];
    return `<div class="status-list__item" data-infection-status-effect="${character.id}"><strong>${effect.icon} ${escapeHtml(effect.name)}</strong><p>${escapeHtml(effect.description)}</p></div>`;
  }

  function customStatusListMarkup(character) {
    return (character.customStatuses || [])
      .map((status) => {
        const title =
          [status.bodyPart, status.severity].filter(Boolean).join(" · ") ||
          "관리자 추가 상태";
        return `<div class="status-list__item"><strong>＋ ${escapeHtml(title)}</strong>${status.detail ? `<p>${escapeHtml(status.detail)}</p>` : ""}</div>`;
      })
      .join("");
  }
  ...
  function renderPlayerProfile() {
    ...
    const systemStatuses = character.statuses
      .map((statusId) => {
        const status = STATUS_DEFINITIONS[statusId];
        return status
          ? `<div class="status-list__item"><strong>${status.icon} ${escapeHtml(status.name)}</strong><p>${escapeHtml(status.description)}</p></div>`
          : "";
      })
      .join("");
    const statuses =
      `${infectionStageStatusMarkup(character)}${systemStatuses}${customStatusListMarkup(character)}` ||
      emptyStateMarkup("현재 적용된 상태이상이 없습니다.");
    const movementCard =
      character.role === "spirit"
        ? `<div class="stat-card"><span>행동력</span><strong>${character.ap} / ${character.maxAp}</strong></div>`
